...
payload = {
    ...
    "queryObject": {
        "columns": [
            ...
            {
                "name": "page_category_text_5",
                "scope": "OBJECT",
                "context": "PAGE"
            },
            {
                "name": "page_parameter_text_52",
                "scope": "OBJECT",
                "context": "PAGE"
            },
            {
...
#payload = replace_time_filter(payload, "2025-11-05 00:00:00", "2025-11-12 00:00:00")
...
if not resultUrl:
    ...
    df.write.format("delta") \
        ...
        .saveAsTable("swi_audience_prd.swiplus.mapp_api_query_7241_13")
...
df = spark.table("swi_audience_prd.swiplus.mapp_api_query_7241_13")

# Add a row number, ordering by a monotonically increasing id to preserve order
df_with_id = df.withColumn(
    "row_id",
    monotonically_increasing_id()
)

# Keep the last occurrence of each "pages" value
w_desc = Window.partitionBy("pages").orderBy(df_with_id["row_id"].desc())
df_dedup = df_with_id.withColumn(
    "rn",
    row_number().over(w_desc)
).filter("rn = 1").drop("row_id", "rn")

display(df_dedup)

df_dedup.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable("swi_audience_prd.swiplus.mapp_api_query_7241_13")